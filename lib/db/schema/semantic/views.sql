...
-- Create semantic schema if not exists
...
WITH period_transactions AS (
  SELECT 
    ...
    SUM(CASE 
      ...
      WHEN ca.type = 'expense' THEN jl.debit - jl.credit
      ELSE 0
    END) AS amount
  FROM journal_entries je
  JOIN journal_lines jl ON jl.journal_entry_id = je.id
...
SELECT 
  ...
  ca.normal_balance,
  CASE 
    WHEN ca.type IN ('asset', 'expense') AND ca.normal_balance = 'debit' THEN ca.current_balance
    WHEN ca.type IN ('liability', 'equity', 'income') AND ca.normal_balance = 'credit' THEN ca.current_balance
    ELSE -ca.current_balance
...
expenses AS (
  SELECT 
    COALESCE(SUM(b.total_amount), 0) AS total_expenses,
...
SELECT 
  ...
  CASE 
    ...
    WHEN il.quantity_available < COALESCE(il.reorder_point, p.reorder_point, 0) THEN 'low_stock'
    WHEN il.quantity_available > COALESCE(il.max_stock_level, il.reorder_point * 3, 999999) THEN 'overstock'
    ELSE 'in_stock'
  END AS stock_status,
...
SELECT 
  ...
  so.total_quantity_ordered - so.total_quantity_fulfilled AS pending_quantity,
  CASE 
...
SELECT 
  ...
  po.total_quantity_ordered - po.total_quantity_received AS pending_quantity,
  CASE 
...
current_assets AS (
  SELECT COALESCE(SUM(current_balance), 0) AS total_current_assets
  FROM chart_accounts
  WHERE organization_id = auth_org_id()
    AND type = 'asset'
...
current_liabilities AS (
  SELECT COALESCE(SUM(current_balance), 0) AS total_current_liabilities
  FROM chart_accounts
  WHERE organization_id = auth_org_id()
    AND type = 'liability'
...
equity AS (
  SELECT COALESCE(SUM(current_balance), 0) AS total_equity
  FROM chart_accounts
  WHERE organization_id = auth_org_id()
    AND type = 'equity'
),
revenue_ytd AS (
  SELECT COALESCE(SUM(credit - debit), 0) AS total_revenue
...
-- Grant permissions to these views
